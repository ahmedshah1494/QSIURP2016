...
class hmm:
	...
	def train(self,nIters, obsSeqs):
		def getBestBPS(i,j,obsSeq):
			sys.stdout.write("\rname: %s 	state: %d 	obsSeq#: %d" % (self.name, i,j))
			sys.stdout.flush()
			BPS = self.getMemTable()
			pathScore = self.bestPathScr(i,len(obsSeq) - 1, obsSeq,BPS)
			# print pathScore, BPS
			self.BPSs.append(BPS)
			self.PathScores.append(pathScore)
		...
		for it in range(nIters):
			...
			for j in range(len(obsSeqs)):
				...
				if (it > 0):
					...
					threadPool = []
					for i in range(self.nStates):
						t = threading.Thread(target=getBestBPS,args=(i,j,obsSeq))
						t.start()
						threadPool.append(t)
					isNotDone = reduce(lambda x,y : x or y, map(lambda x : x.isAlive(),threadPool))	
					while (isNotDone):
						l = map(lambda x : x.isAlive(),threadPool)
						# print l
						isNotDone = reduce(lambda x,y : x or y, l)	
					bestBPSidx = np.argmax(self.PathScores)
					bestBPS = self.BPSs[bestBPSidx]
					# print bestPathScore
					# print bestBPS
...
def trainHMM(c):
	# sys.stdout.write("\r%s %s" % ('training',c))
	# sys.stdout.flush()
	HMMs[c] = hmm(nStates,c)
	HMMs[c].train(nIters,getObsSeq('files/%s/' % c))

threadPool = []
for c in train_rooms:
	t = threading.Thread(target=trainHMM,args=(c,))
	t.start()
	threadPool.append(t)
isNotDone = reduce(lambda x,y : x or y, map(lambda x : x.isAlive(),threadPool))	
while (isNotDone):
